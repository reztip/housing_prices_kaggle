...
Because the distribution of log price appears relatively gaussian, we may want to predict log-price to better fit regression model theory.
...
X = data.matrix(select(train.df, -SalePrice))
...
# Predictions
```{r}
X.test = data.matrix(select(test.df, -Id))
